...
temp_reg <- lm(pie_crab$water_temp ~ pie_crab$latitude)
...
the R^2^ is 0.347 while the R\^2 for water temp is 0.916 so the water temp regression is a better fit. This is intuitive when looking at the graphs, because the water temperature has a more linear fit. \

calculating SSE

```{r}
size_SSE <- sum(resid(lm(pie_crab$size ~ pie_crab$latitude))^2)
# variance relative to the fitted line, that's why we use the regression

size_SST <- sum((pie_crab$size - mean(pie_crab$size))^2)
# variance relative to the mean of the variable, that's why we use the actual variable
...
# the line in the middle is the median, which is pretty insensitive to outliers. the bulk of the data from both years are pretty similar because the data lie in the same quartile
...
2.  What does the intercept tell you, in words?
    ...
    The intercept is 42.46 which is the hwy mileage at year 1999

    the intercept occurs when vintage = 0 which is when the year is 1999 (because R assigned the values to this categorical variable 0 for 1999 and 1 for 2008).

    intercept is the value at the reference level and the coefficients are the increases relative to that reference level

3.  What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? **Answers:**

```{r}
summary(lm(mpg$hwy ~ mpg$year_fct))
```
