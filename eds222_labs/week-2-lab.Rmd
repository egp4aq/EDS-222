...
```{r}
crab <- lterdatasampler::pie_crab
ggplot(data = crab, aes(x = latitude, y = size)) +
  geom_point()
```

```{r}
ggplot(data = crab, aes(x = latitude, y = water_temp)) +
  geom_point()
```
...
(i) the size of crab and latitude: 0.3
...
2.  Use the `cor()` function in `R` to calculate the correlation coefficient between the two variables in (i) and the two in (ii). How close was your guess to the true answer?
    ...
    My guess for the first answer was much lower than the actual correlation.

```{r}
cor(crab$latitude, crab$size)
cor(crab$latitude, crab$water_temp)
```
...
2.  Interpret your two coefficients, paying careful attention to units. What do $\hat{\beta_0}$ and $\hat{\beta_1}$ represent?
    ...
    \^β0 represents the intercept of the lm line where B0 without the hat is the size at latitude = 0 aka the equator.

3.  Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$?

    it is only directly interpretable if you are thinking in context of the axes we're working with (i.e. latitude and not time)

    the \^β0 for the size is not interpretable because it is negative. again, thinking of it in context, we know it's not interpretable because size cannot be negative. \^β1 is more directly interpretable. Here, it means that when we increase 0.4851 degrees latitude, we gain 1 mm of size.

```{r}
lm(size ~ latitude, crab)
lm(water_temp ~ latitude, crab)
```
...
note to self: geom_smooth() inherits its aesthetics from the parent (ggplot). by default, the formula is y \~ x. se is the standard error represented with a shaded area around the line. for now, we're going to have it as false.
...
ggplot(crab, aes(latitude, water_temp)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) +
  theme_bw()
```
...
```{r}
ggplot(crab2, aes(size_residual)) +
  geom_histogram()
```
...
```{r}
ggplot(crab2, aes(x = latitude, y = size_residual)) +
  geom_point() +
